...
def inject_lissajous_line(grid_surface: pygame.Surface, t: float, color_shift: float, endpoint_speed: float) -> None:
    c = (GRID_SIZE - 1) * 0.5
    s = endpoint_speed
    ...
    draw_aa_subpixel_line(grid_surface, x1, y1, x2, y2, t, color_shift)
    endpoint_color_a = rainbow_color_with_phase(t, color_shift, 0.0)
    endpoint_color_b = rainbow_color_with_phase(t, color_shift, 1.0)
    draw_aa_endpoint_disc(grid_surface, x1, y1, endpoint_color_a, radius=0.85)
    draw_aa_endpoint_disc(grid_surface, x2, y2, endpoint_color_b, radius=0.85)
...
def main() -> None:
    ...
    sliders = [
        ...
        Slider("Y Amplitude", 0.10, 1.00, 1.00, pygame.Rect(ui_x + 20, slider_y(4), RIGHT_UI_WIDTH - 40, 14), 2),
        Slider("Y Frequency", 0.10, 4.00, 0.32, pygame.Rect(ui_x + 20, slider_y(5), RIGHT_UI_WIDTH - 40, 14), 2),
        Slider("Endpoint Speed", 0.00, 2.00, ENDPOINT_SPEED_DEFAULT, pygame.Rect(ui_x + 20, slider_y(6), RIGHT_UI_WIDTH - 40, 14), 2),
        Slider("Color Shift", 0.00, 1.00, COLOR_SHIFT_DEFAULT, pygame.Rect(ui_x + 20, slider_y(7), RIGHT_UI_WIDTH - 40, 14), 2),
        Slider("Fade %", 90.0, 99.999, 99.922, pygame.Rect(ui_x + 20, slider_y(8), RIGHT_UI_WIDTH - 40, 14), 3),
    ]
    ...
    while running:
        ...
        x_speed, x_amp, x_scale, y_speed, y_amp, y_scale, endpoint_speed, color_shift, fade_percent = [s.value for s in sliders]
        ...
        draw_x_graph(screen, x_graph_rect, x_profile, x_amp, "x controls columns", font)
        draw_y_graph(screen, y_graph_rect, y_profile, y_amp, "y controls rows", font)
